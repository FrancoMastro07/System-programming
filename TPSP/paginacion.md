...
-Tanto en el page directory entry como en el page table entry tenemos un bit denominado "U/S (User/Supervisor)", en el cual, si lo seteamos en 0 decimos que un proceso se encuentra en modo usuario, y de otro modo se encontraria en modo del sistema.
...
-Ya que en el taller usamos segmentación + paginación tenemos varios pasos a tener en cuenta:
    -La direccion logica se divide en un selector de segmento y un offset
    -Con el selector de segmento se indexa en la GDB y se obtiene la base del segmento
    -Se suma la base del segmento con el offset, obteniendo la direccion lineal
    -La direccion lineal se divide en directorio, tabla y offset
    -El registro CR3 contiene en sus 20 bits mas altos la direccion al directorio
    -En los 10 bits mas significativos de la direccion lineal, esta el offset con el que indexamos en el directorio, para obtener la direccion de la tabla de paginas
    -Nuevamente, en los 10 bits del medio de la direccion lineal, encontramos el offset para indexar en la tabla de paginas, obtenemos la direccion de la pagina
    -Finalmente, con los 12 bits menos significativos de la direccion lineal obtenemos el offset que le debemos sumar a la direccion de la pagina para encontrar la direccion fisica buscada.

c)  ¿Cuál es el efecto de los siguientes atributos en las entradas de la
    tabla de página?

  - `D` = Indica si se escribió con anterioridad por esta PTE, lo escribe el procesador
  - `A` = Indica si se accedió a memoria por esta PTE, lo escribe el procesador
  - `PCD` = Deshabilita cachear los datos de la pagina asociada
  - `PWT` = Deshabilita hacer write-back al escribir en la pagina asociada
  - `U/S` = Determina si el procesa se encuentra en modo usuario o en modo del sistema
  - `R/W` = Determina si un proceso puede escribir o solo leer a la memoria controlada por esta PTE
  - `P` = Indica que la traduccion es valida
...
-Si los atributos U/S y R/W difieren, se aplica la mas restrictiva, separandolo por casos, si el directorio de paginas tiene U/S = 0 (sistema), entonces todas las paginas dentro de esa tabla sera de sistema, sin importar el valor de U/S en la tabla. En otro caso, si el directorio de paginas tiene R/W = 0 (solo lectura), todas las paginas dentro de esa tabla sera de solo lectura, sin importar el valor de R/W en la tabla. Por ultimo si el directorio de paginas permite mas acceso, la tabla de paginas es la que define los permisos finales.
...
-Considerando que el codigo de la tarea ocupa dos paginas y utilizamos una para la pila, por estructura de paginación necesitamos una pagina para el directorio, otra para las tablas de paginas, dos para el codigo de la tarea y una para la pila, necesitariamos 5 paginas en total para la tarea.
...
-La TLB es una cache de traducciones para acelerar este mismo proceso, actua como una pequeña memoria cache, almacenando las traduccions recientes de direcciones virtuales a fisicas. Es necesario utilizar tlbflush para purgarlo, ya que cuando modificas el directorio o las tablas de paginas, las entradas antiguas quedan obsoletas y si no las invalidas podrian seguir usandose. 
-Cada traduccion en la TLB posee los atributos de la pagina y bits LRU para determinar cual es el elemento que debe ser desalojado.
-Esto no sucede, ya que al TLB ser un caché, invalidar una traduccion del TLB no altera la entrada original en la tabla de paginas, lo unico que sucederia es que su busqueda no seria tan rapida como antes.
...
-Es necesario mapear y desmapear las paginas de destino y fuente ya que al ser direcciones fisicas es necesario mapearlas para realizar la copia, luego las desmapeamos para evitar conflictos en futuras traducciones de memoria. Esos macros son utilizados como direcciones virtuales para mapear las direcciones fisicas. Es necesario obtener el cr3 con rcr3() ya que necesitamos el puntero hacia el directorio que se encuentra en cr3, y se usa rcr3() porque no hay una forma directa de obtener un registro de assembler en C.
