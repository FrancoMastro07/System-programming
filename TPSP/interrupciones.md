...
- El campo offset_31_16 toma los 16 bits mas altos del offset y el offset_15_0 toma los 16 bits mas bajos
- El offset hace referencia al offset que tiene el código que maneja la interrupción dentro del segmento dado por el selector de segmento.
- segsel guarda la dirección del selector de segmento en donde se encuentra el código que se va a ejecutar para manejar la interrupción.
- type hace referencia al tipo de interrupción. Nosotros solo estamos utilizando Interrupt Gate.
- dpl indica el nivel de privilegio mínimo requerido por el código que intenta invocar esta interrupción mediante una instrucción de software como INT n. No aplica para interrupciones generadas por hardware
- present indica si el segmento de código que maneja la interrupción está presente o no.
...
- Elegimos como selector de segmento al escrito en defines.h como GDT_CODE_0_SEL, ya que queremos que el codigo presente tenga permisos de nivel 0, para asi pueda ser ejecutado por el kernel.
- Los atributos usando Gate Size de 32 bits nos quedaron de la siguiente manera:
    - type = INTERRUPT_GATE_TYPE         #Valor para una interrupt gate de 32 bits (0xE)
    - dpl = 0        #Privilegio de nivel 0
    - present = 1     #Entrada de IDT valida
...
- Para las interrupciones de teclado y de reloj utilizariamos el macro de ID_ENTRY0 que crea una IDT con nivel de privilegio de kernel. Cada IDT lleva su numero correspondiente, 32 para el reloj y 33 para el teclado. 
...
- Para el prologo usamos pushad y para el epilogo popad y iret, a diferencia de un codigo assembler convencional, esto es asi ya que queremos preservar los registros de proposito general de la forma mas rapida posible con pushad, y luego restaurarlos con popad, todo en una sola linea. Contestando la siguiente pregunta, usamos iret en vez de ret ya que, ret solo restaura la direccion de retorno, e iret se encarga de restaurar registros indispensables del procesador a su estado antes de la interrupción, mas especificamente los registros EFLAGS, CS y EIP.
