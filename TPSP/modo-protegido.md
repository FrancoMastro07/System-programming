...
- Modo real: Este modo provee el entorno original de programación que tenía el Intel 8086. Se le agregan algunas extensiones como por ejemplo pasar a modo protegido.
- Modo protegido: Modo nativo del procesador. Provee mas herramientas, mas flexibilidad, performance y compatibilidad con software mas antiguo.   
...
- El procesador está diseñado para funcionar en modo protegido. El modo real se supone que sirve para que al arrancar se pueda correr en hardware antiguo. El modo protegido tiene mas funcionalidades y mayor rendimiento.
La idea es deshacernos de los limites que tiene el modo real, como por ejemplo limite de direccionamiento a 1 mega, registros de 16 bits, menor cantidad de instrucciones.
...
- La GDT es una tabla de descripciones de segmento. Tiene un tamaño de 1024 y el primer espacio no se usa.
Los descriptores de segmentos sirven para configurar caracteristicas de un segmento, como por ejemplo tamaño, tipo, etc.
Los segmentos son espacios delimitados se memoria. Con su descripción se puede saber que se puede o no hacer en el, que espacio tiene, etc.

- Los primeros 16 bits son de limite, los siguientes 24 son de base, luego 4 de tipo, 1 de descriptor type, dos de descriptor privilege level, 1 segment present, 4 de limite, 1 de available, 1 para indicar si el segmento es de 64 bits, 1 de default operation size, 1 de granularity, y por último 8 de base.

- Limit: Sirve para especificar el tamaño del segmento. Se juntan los dos campos de limite para que te quede un valor de 20 bits.

- Base: Define donde empieza el segmento dentro de los 4gb disponibles.

- G: La granularidad determina la escala del campo limit. Cuando es 0, cada bit representa un byte. Cuando el flag es 1, cada bit representa 4 Kbytes. Esto permite tener segmentos de mayor tamaño.

- P: Indica si el segmento está presente en memoria o no. Si el flag se setea en clear (no presente), se genera una excepción de segment-not-present.

- DPL: Especifica el nivel de privilegio del segmento. Van de 0 a 3, pero solo se usan el 0 (el mas privilegiado) y 3 (el menos privilegiado).

- S: Si el flag es 0, significa que es un segmento de sistema. Si es 1 significa que es un segmento de código o data.
...
- Tendría que ser 1010 Execute/Read
...
- [*GDT Entradas completadas*](https://docs.google.com/spreadsheets/d/1G0z3zRKZzC-1W6xwGsOu4AzHXYogONuQ6Vz0B4eeIrQ/edit?usp=sharing)
...
- LGDT sirve para cargar el registro GDTR. Cuando el procesador se enciende, la dirección base se setea en 0 y el limit en 0xFFFF. Se debe cargar una nueva dirección base en el proceso de pasaje a modo protegido.
...
- El registro CR0 contiene varios bits de control, es necesario para pasar a modo protegido ya que es necesario setear su bit numero 0 en 1 para realizar el pasaje.
Este registro no se puede modificar directamente, hay que utilizar mov con otro registro y modificar ese, luego volver a realizar un mov al CR0.

    bits del 0 al 31:

    0 -> (PE) = 1    (0 en modo real y 1 en modo protegido)

    1 -> (MP) = 0     (0 si no se usa multitarea con FPU)

    2 -> (EM) = 0      (0 si se usa FPU real, 1 si no hay FPU fisica)

    3 -> (TS) = 0       (se pone en 1 automaticamente cuando hay un cambio de tarea)

    4 -> (ET) = 1      (1 para soporte para intel)

    5 -> (NE) = 1       (1 para habilitar el mecanismo de reporte de errores)

    6-15 -> RESERVADO

    16 -> (WP) = 0      (0 para escribir en paginas read-only)

    17 -> RESERVADO

    18 -> (AM) = 0        (0 para deshabilitar el control de exepciones por error de alineacion)

    19-28 -> RESERVADO

    29 -> (NW) = 0        (0 para escribir en cache)

    30 -> (CD) = 0        (0 para activar la cache)

    31 -> (PG) = 0      (0 si no usamos paginacion, 1 si si lo usamos)
...
- De todos los jmp que existen, el adecuado para usar es el Far jump, ya que permite saltar a otro segmento modificando CS y EIP. El indirect jump tambien puede ser valido porque como la direccion esta en memoria o en un registro, puede ser near o far.
...
- Considerando lo descrito en el párrafo anterior, el tamaño deberá ser igual al producto entre cantidad de filas, de columnas y el tamaño de cada celda. Esto es igual a 8000 bytes. Para setear el límite, como este es el offset máximo le debemos restar uno a este valor.
...
- Lo que creemos que hace el método screen_draw_box es cargar con el caracter y el atributo dado, pixeles en un rango indicado por la fila inicial, la columna inicial y la cantidad de filas y columnas. El acceso a memoria se hace a través de la dirección física del buffer de video. Esta se iguala al puntero p, y se sabe que a partir de la misma se encuentra la información que se muestra en la pantalla. Esto debido a que la memoria está almacenada de manera contigua. La estructura que se usa para representar cada caracter de la pantalla y cuanto ocupa en memoria es un array de arrays de la estructura ca. Esto se define a través del tipo (ca(*)[VIDEO_COLS]). ca tiene un largo de 2 bytes donde en uno contiene la información del caracter, mientras que en el restante su atributo. VIDEO_COLS es el largo de cada fila del array multidimensional que representa la pantalla.
...
- Lo que nos pareció interesante en general es ver la cantidad de código que es necesario solamente para arrancar el kernel.

    También como se puede generalizar la creación de descriptores de segmento, haciendo que termine siendo mas cómodo definirlos.
